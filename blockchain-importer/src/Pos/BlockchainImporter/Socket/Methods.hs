...
-- | Logic of BlockchainImporter socket-io Server.

module Pos.BlockchainImporter.Socket.Methods
       ( Subscription (..)
       , ClientEvent (..)
       , ServerEvent (..)
       , SubscriptionParam (..)
...
import           Pos.BlockchainImporter.Socket.Holder (ClientContext, ConnectionsState, BlockchainImporterSocket (..),
                                             BlockchainImporterSockets, ccAddress, ccConnection,
...
import           Pos.BlockchainImporter.Socket.Util (EventName (..), emitTo)
import           Pos.BlockchainImporter.Web.ClientTypes (CAddress, CTxBrief, CTxEntry (..), EpochIndex (..),
                                               TxInternal (..), fromCAddress, toTxBrief)
import           Pos.BlockchainImporter.Web.Error (BlockchainImporterError (..))
import           Pos.BlockchainImporter.Web.Server (getBlocksLastPage, getEpochPage, getEpochPagesOrThrow,
                                          topsortTxsOrFail)
...
broadcast
    :: (BlockchainImporterMode ctx m, EventName event, ToJSON args)
    => event -> args -> Set SocketId -> BlockchainImporterSockets m ()
...
notifyAddrSubscribers
    ...
    => Address -> [CTxBrief] -> BlockchainImporterSockets m ()
