...
-- | `BlockchainImporterSockets` monad.

module Pos.BlockchainImporter.Socket.Holder
       ( BlockchainImporterSockets
       , BlockchainImporterSocket(..)
       , _ProdSocket
       , _TestSocket
...
data BlockchainImporterSocket
    = ProdSocket Socket
    | TestSocket String

makePrisms ''BlockchainImporterSocket

data ClientContext = ClientContext
    { _ccAddress    :: !(Maybe Address)
    , _ccConnection :: !BlockchainImporterSocket
    }

makeClassy ''ClientContext

mkClientContext :: BlockchainImporterSocket -> ClientContext
...
askingConnState
    :: MonadIO m
    => ConnectionsVar
    -> BlockchainImporterSockets m a
    -> m a
askingConnState var action = do
    v <- readTVarIO var
    runReaderT action v

type BlockchainImporterSockets = ReaderT ConnectionsState
