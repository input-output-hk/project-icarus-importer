...
onApplyCallGeneral    blunds = do
    epochBlocks <- onApplyEpochBlocksBlockchainImporter blunds
    pageBlocks  <- onApplyPageBlocksBlockchainImporter blunds
    lastTxs     <- onApplyLastTxsBlockchainImporter blunds
...
onRollbackCallGeneral blunds = do
    epochBlocks <- onRollbackEpochBlocksBlockchainImporter blunds
    pageBlocks  <- onRollbackPageBlocksBlockchainImporter blunds
    lastTxs     <- onRollbackLastTxsBlockchainImporter blunds
    inAssertMode DB.sanityCheckBalances
    pure $ SomeBatchOp [epochBlocks, pageBlocks, lastTxs]
...
-- For @EpochBlocks@
onApplyEpochBlocksBlockchainImporter
    :: MonadBListenerT m
    => OldestFirst NE Blund
    -> m SomeBatchOp
onApplyEpochBlocksBlockchainImporter blunds = onApplyEpochPagedBlocks blunds


-- For @PageBlocks@
onApplyPageBlocksBlockchainImporter
    :: MonadBListenerT m
    => OldestFirst NE Blund
    -> m SomeBatchOp
onApplyPageBlocksBlockchainImporter blunds = onApplyKeyBlocksGeneral blunds pageBlocksMap


-- For last transactions, @Tx@
onApplyLastTxsBlockchainImporter
    :: MonadBListenerT m
    => OldestFirst NE Blund
    -> m SomeBatchOp
onApplyLastTxsBlockchainImporter blunds = generalLastTxsBlockchainImporter blocksNE getTopTxsDiff
  where
...
-- For @EpochBlocks@
onRollbackEpochBlocksBlockchainImporter
    :: MonadBListenerT m
    => NewestFirst NE Blund
    -> m SomeBatchOp
onRollbackEpochBlocksBlockchainImporter blunds = onRollbackEpochPagedBlocks blunds


-- For @PageBlocks@
onRollbackPageBlocksBlockchainImporter
    :: MonadBListenerT m
    => NewestFirst NE Blund
    -> m SomeBatchOp
onRollbackPageBlocksBlockchainImporter blunds = onRollbackGeneralBlocks blunds pageBlocksMap


-- For last transactions, @Tx@
onRollbackLastTxsBlockchainImporter
    :: MonadBListenerT m
    => NewestFirst NE Blund
    -> m SomeBatchOp
onRollbackLastTxsBlockchainImporter blunds = generalLastTxsBlockchainImporter blocksNE getTopTxsDiff
  where
...
class (Ord k) => KeyBlocksOperation k where
    putKeyBlocksF :: (k -> [HeaderHash] -> DB.BlockchainImporterOp)
...
generalLastTxsBlockchainImporter
    :: MonadBListenerT m
    => NE Block
    -> (OldTxs -> NewTxs -> [Tx])
    -> m SomeBatchOp
generalLastTxsBlockchainImporter blocksNE getTopTxsDiff = do
