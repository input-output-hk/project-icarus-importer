...
module Test.Pos.PostgresConsistency.Web.ServerSpec
       ( spec
       ) where
...
import           Pos.PostgresConsistency.BlockchainImporterMode (runBlockchainImporterTestMode)
import           Pos.PostgresConsistency.ExtraContext (ExtraContext (..), makeExtraCtx,
                                                       makeMockExtraCtx)
import           Pos.PostgresConsistency.TestUtil (emptyBlk,
                                                   generateValidBlockchainImporterMockableMode,
                                                   generateValidBlocksSlotsNumber, leftToCounter)
import           Pos.PostgresConsistency.Web.Server (getBlockDifficulty, getBlocksTotal)
...
-- stack test cardano-sl-postgres-consistency --fast --test-arguments "-m Pos.PostgresConsistency.Web.Server"
spec :: Spec
spec = withDefConfigurations $ \_ -> do
    describe "Pos.PostgresConsistency.Web.Server" $ do
        blocksTotalSpec
