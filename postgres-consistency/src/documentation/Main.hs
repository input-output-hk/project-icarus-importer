...
-- | This program builds Swagger specification for PostgresConsistency web API and converts it to JSON.
...
import qualified Pos.PostgresConsistency.Web.Api as A
import qualified Pos.PostgresConsistency.Web.ClientTypes as C
...
main = do
  ...
  where
    jsonFile = "postgres-consistency-web-api-swagger.json"
    ...
    showProgramInfoIfRequired generatedJSON = void $ execParser programInfo
      where
        programInfo = Opt.info (helper <*> versionOption) $
            fullDesc <> progDesc "Generate Swagger specification for PostgresConsistency web API."
                     ...
                     <> footer   ("This program runs during 'cardano-sl' building on CI. " <>
                                  ...
                                  "This documentation will be published at cardanodocs.com using 'update-postgres-consistency-web-api-docs.sh'.")
...
swaggerSpecForBlockchainImporterApi = toSwagger A.blockchainImporterApi
    & info . title       .~ "Cardano SL PostgresConsistency Web API"
    & info . version     .~ toText (showVersion CSLE.version)
    & info . description ?~ "This is an API for Cardano SL PostgresConsistency."
    & host               ?~ "cardano-postgres-consistency.com"
